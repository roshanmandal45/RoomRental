  ...
  return (
    ...
    <nav className='w-full bg-white text-black border-b border-black/5'>
      ...
      <div className='max-w-[1380px] mx-auto px-4 sm:px-8 md:px-12 py-3 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between lg:gap-8'>
        ...
        {/* Desktop Buttons Column (Added "relative" to anchor the absolute dropdown position) */}
        <div className='hidden lg:flex items-center gap-4 shrink-0 relative'>
          <button 
            onClick={() => setBtnOpen(prev => !prev)} 
            className='px-4 py-2 text-sm rounded-full border border-black/10 hover:bg-neutral-50 transition-colors cursor-pointer flex items-center gap-1 font-medium text-neutral-800'
          >
            {selectedButton === "Nepal" ? "🇳🇵 Nepal" : "🇺🇸 United States"}
          </button>

          {btnOpen && (
            <div className='absolute top-12 right-28 w-[230px] bg-white border border-black/10 rounded-xl p-1.5 flex flex-col gap-1 z-50 shadow-lg transition duration-75 animate-fadeIn'>
              <h3 className='text-neutral-400 font-semibold tracking-wider text-[9px] px-3 py-1.5 uppercase'>CHANGE COUNTRY</h3>
              
              {/* Nepal Dropdown Option */}
              <button 
                onClick={() => { setSelectedButton("Nepal"); setBtnOpen(false); }} 
              ...
              > 
                <div className='flex items-center gap-2 text-sm'>
                  <span>🇳🇵</span>
                  <span className={selectedButton === "Nepal" ? 'text-red-500' : 'text-neutral-700'}>Nepal</span>
                </div>
                {selectedButton === "Nepal" && <BsDot size={18} className='text-red-500' />}
              </button>

              {/* United States Dropdown Option */}
              <button 
                onClick={() => { setSelectedButton("United States"); setBtnOpen(false); }} 
                className={`cursor-pointer w-full flex items-center py-2 px-3 justify-between rounded-full font-medium transition-colors duration-150
              ...
              > 
                <div className='flex items-center gap-2 text-sm'>
                  <span>🇺🇸</span>
                  <span className={selectedButton === "United States" ? 'text-red-500' : 'text-neutral-700'}>United States</span>
                </div>
                {selectedButton === "United States" && <BsDot size={18} className='text-red-500' />}
              </button>
            </div>
          )}
      ...
      {isOpen && (
        <div className='fixed top-20 left-3 right-3 p-4 flex flex-col z-50 gap-3 border border-black/5 rounded-2xl bg-neutral-50 shadow-xl lg:hidden animate-fadeIn'>
           ...
           {/* Mobile Country Toggles synced with the active selection state */}
           <div className='flex flex-col gap-1.5'>
            <p className='text-neutral-400 font-semibold tracking-wider text-[9px] px-1 mt-3 uppercase'>CHANGE COUNTRY</p>
             
             {/* Mobile Nepal Trigger */}
             <button 
              onClick={() => { setSelectedButton("Nepal"); setIsOpen(false); }}
             ...
             <button 
              onClick={() => { setSelectedButton("United States"); setIsOpen(false); }}
              className={`w-full flex items-center py-2.5 px-4 justify-between rounded-xl border border-black/5 font-medium transition-colors
